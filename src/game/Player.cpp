...
bool Player::Create(uint32 guidlow, const std::string& name, uint8 race, uint8 class_, uint8 gender, uint8 skin, uint8 face, uint8 hairStyle, uint8 hairColor, uint8 facialHair, uint8 /*outfitId */)
{
    ...
    if (!info)
    {
        sLog.outError("%s have incorrect race (%u) / class (%u) pair. Can't be loaded.", GetGuidStr().c_str(), race, class_);
        return false;
    }
    ...
    if (!cEntry)
    {
        sLog.outError("%s has class %u not found in DBC (Wrong DBC files?)", GetGuidStr().c_str(), class_);
        return false;
    }
    ...
    {
        sLog.outError("%s has invalid gender %u at player creating", GetGuidStr().c_str(), uint32(gender));
        return false;
    }
...
bool Player::BuildEnumData(QueryResult* result, WorldPacket* p_data)
{
    ...
    if (!info)
    {
        sLog.outError("Player %s (Guid: %u) has incorrect race (%u) / class (%u) pair. Don't build enum.", fields[1].GetString(), guid, pRace, pClass);
        return false;
    }
...
bool Player::TeleportTo(WorldLocation const& loc, uint32 options)
{
    ...
    {
        sLog.outError("TeleportTo: %s has invalid map %d or absent instance template.", GetGuidStr().c_str(), loc.GetMapId());
        return false;
    }
...
bool Player::addSpell(uint32 spell_id, bool active, bool learning, bool dependent, bool disabled)
{
    ...
    if (!spellInfo)
    {
        ...
        {
            sLog.outError("Player::addSpell: nonexistent in SpellStore spell (id: %u) request, deleting for all characters in `character_spell`.", spell_id);
            CharacterDatabase.PExecute("DELETE FROM character_spell WHERE spell = %u", spell_id);
        }
        else
            sLog.outError("Player::addSpell: %s. Nonexistent in SpellStore spell (id: %u) request.", GetGuidStr().c_str(), spell_id);
    ...
    if (!SpellMgr::IsSpellValid(spellInfo,this,false))
    {
        ...
        if (!IsInWorld() && !learning)                       // spell load case
        {
            sLog.outError("Player::addSpell: Broken spell (id: %u) learning not allowed, deleting for all characters in `character_spell`.", spell_id);
            CharacterDatabase.PExecute("DELETE FROM character_spell WHERE spell = %u", spell_id);
        }
        else
            sLog.outError("Player::addSpell: %s. Broken spell (id: %u) learning not allowed.", GetGuidStr().c_str(), spell_id);
...
void Player::BuildPlayerRepop()
{
    ...
    if (GetCorpse())
    {
        sLog.outError("BuildPlayerRepop: %s already has a corpse", GetGuidStr().c_str());
        sLog.outError("Removing player %s corpse from DB", GetGuidStr().c_str());
    ...
    if (!corpse)
    {
        sLog.outError("Error creating corpse for %s", GetGuidStr().c_str());
        return;
    }
    GetMap()->Add(corpse);
    ...
    else if (currVal)                                        // add
    {
        for (int i = 0; i < PLAYER_MAX_SKILLS; ++i)
        {
            if (!GetUInt32Value(PLAYER_SKILL_INDEX(i)))
            {
                ...
                if (!pSkill)
                {
                    sLog.outError("%s has skill not found in SkillLineStore: skill (id: %u)", GetGuidStr().c_str(), id);
                    return;
                }
...
InstancePlayerBind* Player::BindToInstance(DungeonPersistentState* state, bool permanent, bool load, bool extend)
{
    if (state)
    {
        ...
        if (!load)
            DEBUG_LOG("Player::BindToInstance: %s is now bound to map %d, instance %d, difficulty %d", GetGuidStr().c_str(), save->GetMapId(), save->GetInstanceId(), save->GetDifficulty());
...
void Player::_LoadSkills(QueryResult* result)
{
    ...
    if (result)
    {
        do
        {
            ...
            if (!pSkill)
            {
                sLog.outError("%s has skill (id: %u) that does not exist.", GetGuidStr().c_str(), skill);
                continue;
            }
            ...
            if (value == 0)
            {
                sLog.outError("%s has skill (id: %u) with value 0. Will be deleted.", GetGuidStr().c_str(), skill);
            ...
            {
                sLog.outError("%s has more than %u skills.", GetGuidStr().c_str(), PLAYER_MAX_SKILLS);
                break;
            }
        }
        while (result->NextRow());
